...
|![image](img/gateway-widget.png)| A button to setup a tunnel to a preconfigured target address & port via a gateway.<br />When protocol is set to HTTP(S), it will open a new tab to display the webcontent served on that location.<br />Requires an operational gateway tunneling setup, [see](./070-gateway-tunnelling-setup.md). |
